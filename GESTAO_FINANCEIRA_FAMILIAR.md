...
Quando um movimento bancário corresponder a uma ocorrência já materializada de uma recorrência, o usuário poderá vinculá-lo explicitamente na conciliação mesmo que o valor efetivamente pago ou recebido ou a conta utilizada sejam diferentes do previsto. A pesquisa manual por recorrência não deve exigir a mesma conta do extrato; deve mostrar a conta prevista para dar contexto ao usuário. Ao confirmar, apenas a ocorrência selecionada deve ser ajustada para o valor e a conta reais do extrato, marcada como exceção da recorrência e liquidada na data do movimento bancário. O valor-base, a conta padrão da recorrência e as ocorrências futuras permanecem inalterados. Esse vínculo reaproveita o lançamento existente e não cria uma nova despesa ou receita.
